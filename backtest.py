...
#     max_step = config.get("max_regime_step", None)
#
#     regime_list = list(config["regimes"].keys())  # R1 → R6 etc.
#
#     # ------------------------------------------------------------
#     # PRICE DATA
#     # ------------------------------------------------------------
#     price_df = price_data[tickers].copy()
#     valid_start_date = price_df.dropna().index.min()
#     log(f"[NORMALIZATION] First common valid date: {valid_start_date.date()}")
#
#     # ------------------------------------------------------------
#     # NORMALIZED VALUES
#     # ------------------------------------------------------------
...
#
#     for t in tickers:
#         base_price = price_df.loc[valid_start_date, t]
#         norm = (price_df[t] / base_price) * starting_val
#         norm[:valid_start_date] = np.nan
...
#
#     first_dd = dd_series.loc[valid_start_date]
...
#     shares = rebalance_fn(portfolio_value, alloc, price_df.loc[valid_start_date])
#
...
#                 "Market_Regime": None,
#                 "Portfolio_Regime": previous_regime,
#                 "Rebalanced": "",
...
#
#         # Portfolio regime updates ONLY on rebalance day
#         rebalanced_flag = ""
#
#         # ------------------------------------------------------------
#         # REBALANCE DAY
...
#             # -----------------------------------------------
#             # NEW PORTFOLIO REGIME LOGIC
#             # -----------------------------------------------
#
#             prev_num = regime_number(portfolio_regime)
#             market_num = regime_number(market_regime)
#
#             # RULE 1 — Follow the market DOWN immediately
#             if market_num > prev_num:
#                 new_num = market_num
#
#             # RULE 2 — Only follow market UP when fully recovered to R1
#             elif market_num == 1:
#                 new_num = 1
#
#             # RULE 3 — Otherwise HOLD the portfolio regime
#             else:
#                 new_num = prev_num
#
#             portfolio_regime = regime_label(new_num)
#
#             # Apply updated allocation
#             alloc = get_allocation_for_regime(portfolio_regime, config)
...
#
#             # -------------------------------
#             # QoQ Performance Calculations
#             # -------------------------------
#             qoq_return = (portfolio_value / last_rebalance_value) - 1
#
#             if len(current_quarter_returns) > 1:
#                 qoq_volatility = pd.Series(current_quarter_returns).std()
#             else:
#                 qoq_volatility = 0
#
#             quarter_rec = {
...
#
#         # Daily return for QoQ volatility
#         if prev_value != 0:
